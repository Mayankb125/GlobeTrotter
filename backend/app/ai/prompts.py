...
def build_planning_prompt(
    ...
    if isinstance(end_date, str):
        ...
        if isinstance(start_date_obj, datetime) and isinstance(end_date, datetime):
            num_days = (end_date - start_date_obj).days
        else:
            num_days = 7
...
- Budget: {currency} {float(budget_min):,.0f} - {float(budget_max):,.0f}
...
1. **Daily Schedule**: For each day, include exactly 2 key activities/attractions with:
   - Time slots (e.g., 9:00 AM - 11:00 AM)
   - Activity name and a very short description (max 1 sentence)
   - Location details
...
2. **Accommodation**: Recommend a specific hotel/stay with:
   ...
   - Why it's suitable (max 1 sentence)

3. **Meals**: Suggest exactly 1 restaurant per day with cuisine type and budget

4. **Transportation**: Include how to get from {home_location} to {destination} and local transport
...
5. **Total Cost Breakdown**:
   ...
   - **Grand Total** (must be within budget: {currency} {float(budget_min):,.0f} - {float(budget_max):,.0f})

6. **Important Notes**: Any tips, warnings, or special considerations (max 2 sentences)
...
def build_optimization_prompt(
    ...
    def _json_default(o: Any):
        if hasattr(o, 'quantize') or hasattr(o, 'real'): # Decimal or number types
            return float(o)
